...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    ##########
    # L4 -> L3
    l4_conv_1x1_transpose = tf.layers.conv2d_transpose(l7_l4_sum, num_classes, 4, strides = 2, padding = 'SAME'
        ,kernel_initializer= tf.random_normal_initializer(stddev=0.01)
        , kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-03))
    ...
    #Combine the output of two layers. The first output is the output of the current layer.
    #The second output is the output of a layer further back in the network, typically a pooling layer.
    l4_l3_sum = tf.add(l4_conv_1x1_transpose, l3_conv_1x1)

    #Final Layer
    nn_last_layer = tf.layers.conv2d_transpose(l4_l3_sum, num_classes, 16, strides = 8, padding = 'SAME'
        ,kernel_initializer= tf.random_normal_initializer(stddev=0.01)
        , kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-03))

    #tf.Print(nn_last_layer, [tf.shape(output)[1:3]])
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    learning_rate_new = 0.0008    
    for epoch in range(epochs):
        print("Epoch: {}/{} Learning Rate: {}".format(epoch+1,epochs,learning_rate_new))
        for image, label in get_batches_fn(batch_size):
            #
            #if(epoch < epochs/4 ) : learning_rate_new = 0.0009
            #elif(epoch < epochs/2) : learning_rate_new = 0.0005
            #elif(epoch < epochs*3/4) : learning_rate_new = 0.0001
            #elif(epoch < epochs) : learning_rate_new = 0.00001

            _, loss = sess.run([train_op, cross_entropy_loss], 
                feed_dict={input_image:image, correct_label:label,
                keep_prob: 0.5, learning_rate: learning_rate_new})
